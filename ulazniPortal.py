...
crtaj_kvadar(orMj, (14 + dubina, -7, -6), (14 + dubina, 7, -6), orSm, 98,
             1)  # nadvoj terse      ---------------------------
...
for br in range(4):
    crtaj_stepenice(orMj, (br + 20 + dubina, -3, -5 + br), (br + 20 + dubina, -5, -5 + br), orSm, blok_id=109,
                    rel_smjer="meni")

    crtaj_kvadar(orMj, (br + 20 + dubina, -3, -4 + br), (br + 20 + dubina, -5, -2), orSm, AIR.id,
                 0)  # zrak iznad stepenica
    crtaj_stepenice(orMj, (br + 20 + dubina, 3, -5 + br), (br + 20 + dubina, 5, -5 + br), orSm, blok_id=109,
                    rel_smjer="meni")
...
for br in range(-1, -11, -1):  # stepenice lijevo i desno
    crtaj_stepenice(pomOrigin, (br, -sirina, br), (br, -sirina + 1, br), orSm, blok_id=109,
                    rel_smjer="meni")  # stepenice lijevo
    crtaj_stepenice(pomOrigin, (br, sirina, br), (br, sirina - 1, br), orSm, blok_id=109,
                    rel_smjer="meni")  # stepenice desne
...
endStorage (  pomOrigin ,  orSm , iX=-28 , iZ=19 , iY=-17  ,  materijal = 98, dv = 1 , stepenice_mat = 109 )
# iz OK -iX nazad iZ lijevo
time.sleep ( 10 )
sorter_soba ( pomOrigin , pomSm ,  iX=8 , iZ=0 , iY=-7 ,  materijal = 98, dv = 1 , stepenice_mat = 109)
# prvo nacrtaj sobu
time.sleep ( 10 )
crtanje_stepenastiTunel ( pomOrigin  , pomSm ,  visina=5 , sirina = 4 , duzina = 7  , uspon = -1 ) # pa tunel
for br in range ( 7 ) :
   crtaj_stepenice ( pomOrigin , ( br + 2 , -4, -1 -br  )  , ( br + 2 , 4, -1 -br ) ,
        pomSm   , blok_id = 109 , rel_smjer  = "odmene") # pa stepenice
time.sleep ( 10 )
...
for br in range(7):
    crtaj_stepenice(pomOrigin, (br + 2, -4, -1 - br), (br + 2, 4, -1 - br), pomSm, blok_id=109,
                    rel_smjer="odmene")  # pa stepenice
