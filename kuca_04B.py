...
def crtaj_blok(dorigin):
    ...
    crtaj_kvadar(orMj, [0 + dX, 0 + dZ, 3 + dY], [6 + dX, 6 + dZ, 3 + dY], orSm, 5,
                 0)  # blok oak wood strop i eventualno podd ssobe iznad pod

    # sobe grede
    #  uzduzne grede
    crtaj_deblo(orMj, [0 + dX, 0 + dZ, 3 + dY], [0 + dX, 6 + dZ, 3 + dY], orSm, "lijevo_desno", blok_id=17,
                podtip=1)  # prednja
    crtaj_deblo(orMj, [6 + dX, 0 + dZ, 3 + dY], [6 + dX, 6 + dZ, 3 + dY], orSm, "lijevo_desno", blok_id=17,
                podtip=1)  # zadnja
    #  poprecne grede
    crtaj_deblo(orMj, [0 + dX, 0 + dZ, 3 + dY], [6 + dX, 0 + dZ, 3 + dY], orSm, "naprijed_nazad", blok_id=17,
                podtip=1)  # lijeva
    crtaj_deblo(orMj, [0 + dX, 6 + dZ, 3 + dY], [6 + dX, 6 + dZ, 3 + dY], orSm, "naprijed_nazad", blok_id=17,
                podtip=1)  # desna
    #  uspravne grede
    crtaj_deblo(orMj, [0 + dX, 0 + dZ, 0 + dY], [0 + dX, 0 + dZ, 3 + dY], orSm, "gore", blok_id=17,
                podtip=1)  # prednja lijeva
    crtaj_deblo(orMj, [6 + dX, 0 + dZ, 0 + dY], [6 + dX, 0 + dZ, 3 + dY], orSm, "gore", blok_id=17,
                podtip=1)  # zadnja lijeva
    crtaj_deblo(orMj, [0 + dX, 6 + dZ, 0 + dY], [0 + dX, 6 + dZ, 3 + dY], orSm, "gore", blok_id=17,
                podtip=1)  # prednja desna
    crtaj_deblo(orMj, [6 + dX, 6 + dZ, 0 + dY], [6 + dX, 6 + dZ, 3 + dY], orSm, "gore", blok_id=17,
                podtip=1)  # zadnja desna
    ...
    # crtaj prozore
    if dY > -1:
        crtaj_kvadar(orMj, [0 + dX, 2 + dZ, 1 + dY], [0 + dX, 4 + dZ, 1 + dY], orSm, 102,
                     0)  # staklo za prozor prednji
...
for ha in (-3, 1, 5, 9, 13):
    ...
    crtaj_stepenice(orMj, (10, - 7, ha + 2), (10, -8, ha + 2), orSm, blok_id=53,
                    rel_smjer="odmene")  # cetvrta stepenica
...
for br in range(0, 4):
    # gornji krov
    crtaj_stepenice(orMj, (7 + br, - 10 + br, 5 + 15 + br), (7 + br, - 2 - br, 5 + 15 + br), orSm, blok_id=134,
                    rel_smjer="meni")  # prednja kosina
    crtaj_stepenice(orMj, (15 - br, - 10 + br, 5 + 15 + br), (15 - br, - 2 - br, 5 + 15 + br), orSm, blok_id=134,
                    rel_smjer="odmene")  # zadnja kosina
    crtaj_stepenice(orMj, (7 + br, - 10 + br, 5 + 15 + br), (15 - br, - 10 + br, 5 + 15 + br), orSm, blok_id=134,
                    rel_smjer="lijevo")  # lijeva kosina
    crtaj_stepenice(orMj, (7 + br, - 2 - br, 5 + 15 + br), (15 - br, - 2 - br, 5 + 15 + br), orSm, blok_id=134,
                    rel_smjer="desno")  # lijeva kosina
    # prednji krov
    crtaj_stepenice(orMj, (1 + br, - 10 + br, 7 + br), (1 + br, - 2 - br, 7 + br), orSm, blok_id=134,
                    rel_smjer="meni")  # prednja kosina
    # crtaj_stepenice ( orMj , ( 15 + br , - 10 - br , 7 + br) , ( 15 + br , - 2 - br , 7 + br ) , orSm , blok_id = 53 , rel_smjer  = "odmene" ) # zadnja kosina
    crtaj_stepenice(orMj, (1 + br, - 10 + br, 7 + br), (7, - 10 + br, 7 + br), orSm, blok_id=134,
                    rel_smjer="lijevo")  # lijeva kosina
    crtaj_stepenice(orMj, (1 + br, - 2 - br, 7 + br), (7, - 2 - br, 7 + br), orSm, blok_id=134,
                    rel_smjer="desno")  # lijeva kosina
    # desni krov
    crtaj_stepenice(orMj, (7 + br, - 2, 7 + br), (7 + br, 10 - br, 7 + br), orSm, blok_id=134,
                    rel_smjer="meni")  # prednja kosina
    crtaj_stepenice(orMj, (15 - br, - 2, 7 + br), (15 - br, 10 - br, 7 + br), orSm, blok_id=134,
                    rel_smjer="odmene")  # zadnja kosina
    # crtaj_stepenice ( orMj , ( 7 + br , - 10 + br , 15 + br) , ( 15 - br , - 10 + br , 15 + br ) , orSm , blok_id = 134 , rel_smjer  = "lijevo" ) # lijeva kosina
    crtaj_stepenice(orMj, (7 + br, 10 - br, 7 + br), (15 - br, 10 - br, 7 + br), orSm, blok_id=134,
                    rel_smjer="desno")  # lijeva kosina

crtaj_kvadar(orMj, [11, -6, 18 + 5], [11, -6, 18 + 5], orSm, 126,
             9)  # toranj - slabovi za zatvoriti rupu u sredini krova   wooden Upper Oak Wood Slab
crtaj_kvadar(orMj, [5, -6, 10], [7, -6, 10], orSm, 126,
             9)  # prednji krov - slabovi za zatvoriti rupu u sredini krova   wooden Upper Oak Wood Slab
crtaj_kvadar(orMj, [11, -2, 10], [11, 6, 10], orSm, 126,
             9)  # desni krov - slabovi za zatvoriti rupu u sredini krova   wooden Upper Oak Wood Slab
