...
def tvrdja(orMj, orSm, iX=0, iZ=0, iY=0, materijal=98, dv=0, stepenice_mat=109):
    ...
    orMj = premjesti_origin(katOrigin, 0, 0, 6,
                            orSm)  # mice ishodiste na centar na vrh katakombi, odatle se crta zgrada
    mc.postToChat("Kocka")
    crtaj_kvadar(orMj, (-dimenzija, -dimenzija, 0), (dimenzija, dimenzija, 50), orSm, materijal,
                 dv)  # kocka - crtanje tijela zgrade
    time.sleep(20)
    for br in range(1, 52, 7):  # crtanje etazi u kocki zgrade
        mc.postToChat("Kocka - rupa %f" % br)
        crtaj_kvadar(orMj, (- dimenzija + 1, -dimenzija + 1, br), (dimenzija - 1, dimenzija - 1, br + 5), orSm, 0,
                     0)  # rupa u kocki
        time.sleep(15)
    ...
    for dummy in (1, 2, 3):
        for dZ in range(-18, 19):
            ...
            if abs(
                    dZ) > 15:  # kutije se crtaju samo u unutrasnjih 31 polja , sa svake strane lijevo i desno idu po 3 sortera idu po
                flag_crtaj_kutije = "ne"
            else:
                flag_crtaj_kutije = "da"

            modul_sorter(kata4Origin, orSm, iX=16, iZ=dZ, iY=0, materijal=98, dv=0, kutija=flag_kutija,
            ...
            if abs(dZ) < 16:  # kat ispod krajnji moduli se crtaju samo od -15 do 15
                modul_kraj(kata4Origin, orSm, iX=16, iZ=dZ, iY=-7, materijal=98, dv=0,
...
def mmob_spawner(orMj, orSm, iX=0, iZ=0, iY=0, materijal=98, dv=0, stepenice_mat=109, prosirenje=5):
    ...
    crtaj_kvadar(orMj, (sirina + prosirenje, - sirina - prosirenje, - 4),
                 (- sirina - prosirenje, sirina + prosirenje, 21), orSm, 0, 0)  # clear the deck
    time.sleep(10)

    # orMj = premjesti_origin ( orMj , iX , iZ , iY ,  orSm )


    crtaj_kvadar(orMj, (sirina + 1, - sirina - 1, - 1), (- sirina - 1, sirina + 1, - 1), orSm, 44,
                 0)  # razbijalica okolo
    time.sleep(10)
    crtaj_kvadar(orMj, (sirina - 2, - sirina + 2, - 1), (- sirina + 2, sirina - 2, - 1), orSm, 89,
                 0)  # glowstone platforma u sredini
    time.sleep(10)
    ...
    for dummy in (1, 2, 3, 4):  # brojalica za okretanje
        ...
        crtaj_kvadar(orMj, (sirina - 2, - sirina + 2, 0), (sirina - 2, sirina - 2, 3 * (visina - 1) + 1 + 45), orSm, 20,
                     0)  # inside glass
        crtaj_kvadar(orMj, (sirina + 1, - sirina - 1, 0), (sirina + 1, sirina + 1, razbijalica - 1), orSm, 20,
                     0)  # outside glass

        # skupljaci ispod

        crtaj_kvadar(orMj, (sirina + 1, - sirina - 1, -2), (sirina - 2, sirina + 1, -2), orSm, 154,
                     smjer_hoppera(orSm, "desno"))  # hopperi spod stoneslaba
        crtaj_kvadar(orMj, (sirina + 1, - sirina - 1, -4), (sirina - 2, sirina + 1, -4), orSm, 89,
                     0)  # glowstonei ispod hoppera
    ...
    if prosirenje == 5:  # samo ako je u tornju
        crtaj_hopper(orMj, (sirina + 1, - sirina - 3, -5), (sirina, - sirina - 3, -5), orSm,
                     "dolje")  # hopperi ispod kutija
        crtaj_hopper(orMj, (sirina + 1, - sirina - 3, -6), (sirina - 1, - sirina - 3, -6), orSm,
                     "meni")  # hopperi ispod kutija
        crtaj_hopper(orMj, (sirina - 2, - sirina - 3, -6), (sirina - 2, - sirina - 14, -6), orSm,
                     "lijevo")  # hopperi ispod kutija
    ...
    for dummy in (1, 2, 3, 4):  # brojalica za okretanje
        ...
        for br in range(0, visina):
            ...
            crtaj_kvadar(orMj, (sirina + 1, - sirina - 1, 0 + 3 * br), (2 + dX + sirina + 1, - sirina - 1, 3 * br + 2),
                         orSm, materijal, dv)  # blokovi sastrane za zamracivanje
            crtaj_kvadar(orMj, (sirina + 1, sirina + 1, 0 + 3 * br), (2 + dX + sirina + 1, sirina + 1, 3 * br + 2),
                         orSm, materijal, dv)  # blokovi sastrane za zamracivanje
        # crtaj_kvadar ( orMj , (  sirina + 1  , sirina + 1, 0 + 3 * br + 3 )  , (  2+dX + sirina + 2 , - sirina - 1 ,  3 * br + 3 ) , orSm ,  materijal , dv ) # puni protiv svijetla
        crtaj_kvadar(orMj, (sirina + 1, sirina + 1, 0 + 3 * br + 3), (2 + dX + sirina + 2, - sirina - 1, 3 * br + 3),
                     orSm, 44, 0)  # poklopac na vrhu stepenica half slab anti spawn
        orSm = ortUlijevo(orSm)  # okret za 90
        time.sleep(35)
        # poklopac na sredini
        # ZAKOMENTIRANO zbog testiranja
    mc.postToChat("Poklopac")
    crtaj_kvadar(orMj, (sirina, sirina, 0 + 3 * br + 3), (- sirina, - sirina, 3 * br + 3), orSm, 44,
                 0)  # poklopac na sredini stepenica half slab anti spawn
    time.sleep(10)
